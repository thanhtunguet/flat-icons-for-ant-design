import {IconDefinition} from '@ant-design/icons-angular';

export const FlatAddToQueueButton: IconDefinition = {
  name: 'flat_add_to_queue_button',
  theme: 'outline',
  icon: `<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
     width="561px" height="561px" viewBox="0 0 561 561" style="enable-background:new 0 0 561 561;" xml:space="preserve">
  <g id="queue">
		<path d="M51,102H0v408c0,28.05,22.95,51,51,51h408v-51H51V102z M510,0H153c-28.05,0-51,22.95-51,51v357c0,28.05,22.95,51,51,51
			h357c28.05,0,51-22.95,51-51V51C561,22.95,538.05,0,510,0z M459,255H357v102h-51V255H204v-51h102V102h51v102h102V255z"/>
	</g>
</svg>`,
};
